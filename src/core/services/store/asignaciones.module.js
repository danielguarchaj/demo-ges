...
export default {
  ...
  mutations: {
    ...
    removeFinal: (state, id) =>
      (state.finales = state.finales.filter(final => final.id !== id)),
    addFinal: (state, newFinal) => state.finales.push(newFinal),

    setParciales: (state, parciales) => (state.parciales = parciales),
    removeParcial: (state, id) =>
      (state.parciales = state.parciales.filter(parcial => parcial.id !== id)),
